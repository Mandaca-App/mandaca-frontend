...
import EnterpriseInput, {
  formatHour,
} from '@/components/editStory/enterpriseInput';
...
export default function EditStory() {
    ...
    const fetchEnterprise = async () => {
      try {
        ...
        setForm({
          ...
          hora_abrir: formatHour(data.hora_abrir),
          hora_fechar: formatHour(data.hora_fechar),
        });
  ...
  const handleChangeForm = (field: keyof EnterpriseForm, value: string) => {
    setForm((prev) => ({
      ...prev,
      [field]: value,
    }));
  ...
  const validateFields = (): boolean => {
    ...
    const specialtyResult = specialtySchema.safeParse({
      especialidade: form.especialidade,
    });

    if (!specialtyResult.success) {
      errors.especialidade = specialtyResult.error.issues[0].message;
    }
    ...
    if (!addressResult.success) {
      errors.endereco = addressResult.error.issues[0].message;
    }
    ...
    if (!phoneResult.success) {
      errors.telefone = phoneResult.error.issues[0].message;
    }

    const openHourResult = openHourSchema.safeParse({
      hora_abrir: form.hora_abrir,
    });

    if (!openHourResult.success) {
      errors.hora_abrir = openHourResult.error.issues[0].message;
    }

    const closeHourResult = closeHourSchema.safeParse({
      hora_fechar: form.hora_fechar,
    });

    if (!closeHourResult.success) {
      errors.hora_fechar = closeHourResult.error.issues[0].message;
    }

    const storyResult = storySchema.safeParse({
      historia: text.trim(),
    });

    if (!storyResult.success) {
      errors.historia = storyResult.error.issues[0].message;
    }
  ...
  const handleSave = async () => {
    try {
      ...
      await updateEnterpriseStory(ENTERPRISE_ID, {
        historia: text.trim(),
        nome: form.nome,
        especialidade: form.especialidade,
        endereco: form.endereco,
      ...
      router.replace('/(mybusiness)/businessOverview');
    } catch (error) {
      console.error(error);
      ...
      >
        <View className="gap-6">
          ...
          <EnterpriseInput
            ...
            onChangeText={(value: string) => handleChangeForm('nome', value)}
          ...
          <EnterpriseInput
            ...
            onChangeText={(value: string) =>
              handleChangeForm('especialidade', value)
            }
          ...
          <EnterpriseInput
            ...
            onChangeText={(value: string) =>
              handleChangeForm('endereco', value)
            }
          ...
          <EnterpriseInput
            ...
            onChangeText={(value: string) =>
              handleChangeForm('telefone', value)
            }
          ...
          <EnterpriseInput
            ...
            value={form.hora_abrir}
            error={fieldErrors.hora_abrir}
            onChangeText={(value: string) =>
              handleChangeForm('hora_abrir', value)
            }
            placeholder="08:00"
            isTime
          />
          ...
          <EnterpriseInput
            ...
            value={form.hora_fechar}
            error={fieldErrors.hora_fechar}
            onChangeText={(value: string) =>
              handleChangeForm('hora_fechar', value)
            }
            placeholder="22:00"
            isTime
          />
