...
type ScannedItem = {
  ...
  historia: string;
  preco: string;
  categoria: 'entrada' | 'prato_principal' | 'sobremesa' | 'bebida' | 'lanche';
};
...
export default function ReviewScanScreen() {
  ...
  const [menuItems, setMenuItems] = useState<ScannedItem[]>(() => {
    try {
  ...
  const handleEdit = (item: ScannedItem, index: number) => {
    router.push({
  ...
  const handleSave = async () => {
    try {
      ...
      await createMenuBulk(enterpriseId, menuItems);

      Alert.alert('Sucesso', 'Cardápio criado com sucesso!');

      router.replace('/(mybusiness)/menu/menu');
    } catch (error) {
      console.error(error);
  ...
  return (
    <Container>
      <Header title="Revisar Cardápio" showBackButton />
      ...
      <ScrollView className="mt-5" showsVerticalScrollIndicator={false}>
        <View className="mb-6">
          ...
          <Text className="text-black/60 mt-2 leading-6">
            Revise os itens encontrados pela inteligência artificial antes de
            adicioná-los ao seu cardápio.
          </Text>
        </View>
        ...
        >
          <Text className="font-semibold text-primary">
            {totalItems}{' '}
            {totalItems === 1 ? 'item encontrado' : 'itens encontrados'}
          </Text>
        </View>
        ...
        {menuItems.map((item, index) => (
          ...
          >
            <View className="gap-2">
              ...
              <Text className="text-black/60 leading-6">{item.historia}</Text>
            </View>
            ...
            <View className="flex-row items-center justify-between mt-5">
              ...
              <View className="bg-primary/10 px-3 py-2 rounded-full">
                <Text className="text-primary font-semibold capitalize">
                  {item.categoria === 'prato_principal'
                    ? 'Prato Principal'
                    : item.categoria}
                </Text>
              </View>
            ...
            <View className="flex-row gap-3 mt-5">
              ...
              >
                ...
                <Text className="text-light font-bold">Editar</Text>
              </Pressable>
              ...
              >
                ...
                <Text className="text-light font-bold">Excluir</Text>
              </Pressable>
            </View>
          </View>
        ))}
