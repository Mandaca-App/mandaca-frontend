...
type Props = {
  report: AIReport;
  onRefresh?: (newReport: AIReport) => void;
};

export const CardList = ({ report, onRefresh }: Props) => {
  ...
  const handleGenerateReport = async () => {
    try {
      ...
      const newReport = await generateReport(report.empresa_id);
      ...
      Toast.show({
        type: 'error',
        text1: 'Erro',
        text2: 'Não foi possível atualizar o relatório.',
      });
    } finally {
      setLoading(false);
    }
  ...
  return (
    <View className="mt-10 gap-8">
      <GenerateReportButton
        ...
        handlePress={handleGenerateReport}
        loading={loading}
      />
